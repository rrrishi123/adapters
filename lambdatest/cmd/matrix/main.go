...
var (
	...
	flagSpec      = flag.String("spec", "lambdatest/spec.json", "provider spec to read endpoints + app ids from")
	...
	flagUpload    = flag.String("upload", "", "prereq step: upload apps then exit. Comma list of kind:path — kind = realDevice|virtualDevice|framework:<espresso-android|xcuit-ios|flutter-android|flutter-ios>. Creds below the boundary; ids cached to .matrix/cache.json")
)
