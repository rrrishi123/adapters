...
```
http-mcp        the WIRE      — two modes — CALL & CHANNEL — via two atoms (http_request, bidi_command). Zero provider awareness.
...
adapters  the PROVIDERS — per-provider spec + runner + upload + catalog + host composers. THIS repo.
```
...
dependency flows one way: a pack depends on the wire's two modes; never the reverse.
...
- **`spec.json`** — declarative map: framework → mode (call|channel), endpoint
...
- **a runner** (`cmd/matrix`) — takes a run request (framework, caps, app, auth
  ...
  the two modes; reads creds **below the boundary** (env / gitignored `auth/`),
  never as a CLI arg.
